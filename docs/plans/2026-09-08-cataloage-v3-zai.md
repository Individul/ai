...
- După deploy, prima întrebare pe GLM a picat cu „Network connection lost” în 497 ms. Diagnostic cu un Worker de test (șters după): R2 citește cele 2,2 MB de text în < 0,3 s; de pe rețeaua Cloudflare corpurile de 2 MB trec; secretul `ZAI_API_KEY` pus la promptul interactiv al `wrangler secret put` avea un singur caracter (cod 22, tasta Ctrl+V), iar Cloudflare respinge local antetul `Authorization` stricat. Pus prin stdin din `.dev.vars`, contextul integral al catalogului „Legislația penală” (2,06 M caractere, 676 k tokeni) a răspuns corect în 42 s, ≈ 156 de credite.
